 ...
 * @returns {Promise<{ summary: Array<object>, nudge: { habit_name: string, habit_name_display: string, streak: number } | null }>}
 */
export async function getHabitSummary(supabase, userId) {
  ...
  if (!rows?.length) return { summary: [], nudge: null };
  ...
  const nudge =
    atRiskForNudge.length > 0
      ? atRiskForNudge.reduce((best, cur) => (cur.streak > best.streak ? cur : best))
      : null;

  return { summary: summaryLast7, nudge };
}
